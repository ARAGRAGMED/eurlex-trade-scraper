...
class EURLexTradeScraperDashboard {
    ...
    formatKeywordMatches(matchDetails) {
        ...
        const total = (matchDetails.measure_keywords?.length || 0) + 
                     (matchDetails.product_keywords?.length || 0) + 
                     (matchDetails.place_company_keywords?.length || 0);
        
        return `<span class="badge badge-outline text-purple">${total} keywords</span>`;
    }
